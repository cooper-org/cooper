...
def test_pipeline_without_cmp(
    ...
    if use_ineq_constraints:
        ...
        default_cg_kwargs = {"constraint_type": constraint_type, "formulation_type": cooper.LagrangianFormulation}

        multiplier0 = cooper.multipliers.DenseMultiplier(num_constraints=1, device=device)
        cg0 = cooper.Constraint(**default_cg_kwargs, multiplier=multiplier0)

        multiplier1 = cooper.multipliers.DenseMultiplier(num_constraints=1, device=device)
