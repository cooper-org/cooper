...
def test_pipeline_with_cmp(Toy2dCMP_problem_properties, Toy2dCMP_params_init, use_multiple_primal_optimizers, device):
    ...
    use_slack_variables = Toy2dCMP_problem_properties["use_slack_variables"]

    if not use_slack_variables:
        pytest.skip("This test is only for the case of slack variables.")

    cmp = cooper_test_utils.Toy2dCMP(
        use_ineq_constraints=use_ineq_constraints, use_slack_variables=use_slack_variables, device=device
    )

    slack_params = list(cmp.extract_slack_params())
    slack_optimizer = torch.optim.SGD(slack_params, lr=0.1)
    primal_optimizers = cooper.utils.ensure_sequence(primal_optimizers) + [slack_optimizer]
    ...
    for param, exact_solution in zip(params, Toy2dCMP_problem_properties["exact_solution"]):
        if not torch.allclose(param, exact_solution):
            breakpoint()
