...
The [Torch Optimizers]{ref}`torch-optimizers` section describes **Cooper** implementations of {py:class}`torch.optim.Optimizer` classes tailored for solving {py:class}`CMP<cooper.cmp.ConstrainedMinimizationProblem>`s that are not available in PyTorch.
