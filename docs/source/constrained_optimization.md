...
:::{admonition} Stopping Conditions
:class: tip

The primal updates are not based solely on the gradient of the loss, but also those of the constraints (weighed by the corresponding multipliers). Therefore, it is common for the loss to *increase* during the initial phase of the optimization, as this is often required to satisfy the constraints. After becoming feasible, however, the loss is expected to decrease until an equilibrium is reached.

The presented algorithm looks for a min-max point of the Lagrangian. Therefore, to ensure convergence, you should check that the gradient of the Lagrangian with respect to the primal variables $\vx$ is close to zero, and that complementary slackness holds for the dual variables: $\vlambda \circ \vg = \vzero$ and $\vmu \circ \vh = \vzero$.
:::
